...
import bluetoothGate.BluetoothGateStrategy;
...
public class MainControl {
	public enum Mode {
		WAIT_FOR_BARCODE, WAIT_FOR_RACE, RUNNING
	};

	public enum Level {
		RACE, BRIDGE, LABYRINTH, BOG, BLUETOOTH, TURNTABLE, PUSHER, SEESAW, PLANKBRIDGE, OPPOSITE_LANE, COLORS, ENDBOSS
	}
	...
	/**
	 * This is the main entry point for our program. We initialize the Robot.
	 * After that, we register a ButtonListener to the ENTER-Button so we will
	 * know when the race begins.
	 */
		...
		Button.ENTER.addButtonListener(new ButtonListener() {
			public void buttonPressed(Button b) {
				...
				case WAIT_FOR_RACE:
					...
					break;
				case RUNNING:
					// If we were trying to complete a level, we stop what we
					// are
					// doing
	...
	/**
	 * The race has begun! We have to take two slalom turns, that's all. Oh
	 * yeah... There could be other robots in the way...
	 */
	...
	/**
	 * We have to cross a bridge. It has no guardrail, so we could fall off very
	 * easily. Use the light sensor to determine if we are on the bridge or just
	...
	public static void bluetoothGate() {
		...
		currentStrategy = new BluetoothGateStrategy(robot, null);
		currentStrategy.start();
	 ...
	 * again. What could possibly go wrong? Since the table adds errors to our
	 * turn demands, we ask him multiple times to turn only small degrees and
	 * hope the errors cancel each other out.
	 */
	...
	/**
	 * Since we know nothing about this, there is only one goal: SURVIVAL! EDIT:
	 * The endboss will be some robot with a claw, trying to overthrow this
	 * robots. Hopefully, we are heavy enough.
	 */
